...
#include "su_complex.suh"

namespace QC_V2 {

namespace SimulatorBirenGPUSVV2TypeDef {
...
class SimulatorBirenGPUSVV2 : public SimulatorV2 {
public:
  SimulatorBirenGPUSVV2(std::uint64_t nQubits_ = 0UL);
  ~SimulatorBirenGPUSVV2() override;
...
private:
  using Unitary1QType = SimulatorBirenGPUSVV2TypeDef::Unitary1QType;
  ...
  // SUPA data structures
  int _mDeviceID{0};
  suDeviceProp _mDeviceProp;

  std::vector<suFloatComplex> _mHostStateVector;
  std::vector<suFloatComplex> _mHostStateVectorBuffer;
  float* _mDeviceOutput{nullptr};

  suFloatComplex* _mDeviceStateVector{nullptr};
  suFloatComplex* _mDeviceStateVectorBuffer{nullptr};

  suEvent_t _mEventStart{nullptr};
  suEvent_t _mEventStop{nullptr};

  suFuncAttributes _mApplyGateOneFunctionAttrs;
  suFuncAttributes _mFindProbabilityOfOutcomeZeroFunctionAttrs;
  suFuncAttributes _mCollapseToOutcomeScaleFunctionAttrs;
  suFuncAttributes _mStoreStateVectorFunctionAttrs;
  suFuncAttributes _mDotStateVectorFunctionAttrs;
  suFuncAttributes _mNormStateVectorFunctionAttrs;

private:
  static std::uint64_t getControlMask(std::vector<std::uint64_t> const& ctrls);
  
  void applyGateOne(
    suFloatComplex matrix[4],
