...
#elif defined(ENABLE_METAX)
class MetaxGPUSVSSASimulatorPython {
public:
    MetaxGPUSVSSASimulatorPython() {
        // 创建解析器和执行引擎
        parser_ = new ssa::SSAParser();
        executor_ = new ssa::SSAExecutor();
    }
...
// 模块初始化
PYBIND11_MODULE(ssa_simulator_cpp_metax_gpu_sv, m) {
    py::class_<MetaxGPUSVSSASimulatorPython>(m, "MetaxGPUSVSSASimulator")
        .def(py::init<>())
        .def("load_ssa_assembly", &MetaxGPUSVSSASimulatorPython::load_ssa_assembly)
        .def("run", &MetaxGPUSVSSASimulatorPython::run)
        .def("reset", &MetaxGPUSVSSASimulatorPython::reset)
        .def("reset_state", &MetaxGPUSVSSASimulatorPython::reset_state)
        .def("get_measurement_reg_names", &MetaxGPUSVSSASimulatorPython::get_measurement_reg_names)
        .def("sample", &MetaxGPUSVSSASimulatorPython::sample, py::arg("shots_count") = 1000)
        .def("get_error", &MetaxGPUSVSSASimulatorPython::get_error)
        .def("expect", &MetaxGPUSVSSASimulatorPython::expect)
        .def("get_state_vector", &MetaxGPUSVSSASimulatorPython::get_state_vector);
}
